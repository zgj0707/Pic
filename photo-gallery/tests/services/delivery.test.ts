...
describe('pre-shoot reference package export', () => {
  let tempDir = ''
  ...
  it('copies in inspiration-board order, reports partial failures, and never writes delivered_at', async () => {
    ...
    const projectId = dbAdapter.insert('projects', { name: '拍摄方案项目', description: null }) || 0
    ...
    const result = exportProjectDelivery(projectId, [photoA, missingPhoto], target, '拍摄方案', 'REF-')

    expect(result.success).toBe(false)
    expect(result.copied).toBe(1)
    expect(result.failed).toBe(1)
    expect(result.results[0].targetPath.endsWith('REF-001.jpg')).toBe(true)
    ...
    expect(dbAdapter.get('SELECT delivered_at FROM photos WHERE id = ?', [photoA])?.delivered_at).toBeNull()
